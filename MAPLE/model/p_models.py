...
class Encoder(Module):
    ...
    def forward(self, embed, latent):
        ...
        for i in range(self.num_layers):
            output = self.layers[i](embed, output)
...
class EncoderLayer(Module):
    ...
    def __init__(self, d_model, nhead, dim_feedforward=1024, dropout=0.1, activation="relu"):
        ...
        self.insertCausalSelfAttn = False
        self._future_mask = torch.empty(0)
    ...
    def forward(self, embed, latent):
        ...
        """
        self_attn_mask = self.buffered_future_mask(latent)
        if self.insertCausalSelfAttn:
            latent2 = self.self_attn(latent, latent, latent, attn_mask=self_attn_mask)[0]
        else:
            latent2 = self.self_attn(latent, latent, latent)[0]
        latent = latent + self.dropout0(latent2)
        latent = self.norm0(latent)
        latent2 = self.cross_attn(embed, latent, latent)[0]
        latent = latent + self.dropout1(latent2)
        latent = self.norm1(latent)
        if hasattr(self, "activation"):
            latent2 = self.linear2(self.dropout(self.activation(self.linear1(latent))))
        else:  # for backward compatibility
            latent2 = self.linear2(self.dropout(F.relu(self.linear1(latent))))
        latent = latent + self.dropout2(latent2)
